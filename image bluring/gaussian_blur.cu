...
//         (1)                         (2)                 (3)
//
// A good starting place is to map each thread to a pixel.
                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  // to sequential reference solution for the exact clamping semantics you should follow.
    ...
    //const int filter_size = filterWidth*filterWidth;
    extern __shared__ float sh_filter[];
    if(threadIdx.x < filterWidth && threadIdx.y < filterWidth){
      sh_filter[threadIdx.x * filterWidth + threadIdx.y] = filter[threadIdx.x * filterWidth + threadIdx.y];
    }
    __syncthreads();

    const int halfWidth = filterWidth/2; 
    float result = 0.0f;
    //row and col after clamping
    ...
    for(int filter_r = -halfWidth; filter_r <= halfWidth; filter_r++){
      for(int filter_c = -halfWidth; filter_c <= halfWidth; filter_c++){
        ...
        image_r = min(max(r + filter_r, 0), numRows - 1);
        image_c = min(max(c + filter_c, 0), numCols - 1);
        const float filter_value = sh_filter[(filter_r + halfWidth) * filterWidth + filter_c + halfWidth];
        const float image_value = inputChannel[image_r * numCols + image_c];
                      ...
                      unsigned char* const blueChannel)
{
  ...
  const int indx = r * numCols + c;
  uchar4 cell = inputImageRGBA[indx];

  redChannel[indx] = cell.x;
  greenChannel[indx] = cell.y;
  blueChannel[indx] = cell.z;
}
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize((numCols + blockSize.y - 1) / blockSize.y, 
                       (numRows + blockSize.x - 1) / blockSize.x);
  ...
  int shared_size = filterWidth * filterWidth * sizeof(float);
